...
def _gemini_request(prompt: str, temperature: float = 0.2, max_tokens: int = 1024, max_retries: int = 3) -> Optional[str]:
    ...
    # If recent 429 was detected, skip Gemini calls briefly and fall back fast.
    now = time.time()
    if now < _gemini_cooldown_until:
        remaining = int(_gemini_cooldown_until - now)
        print(f"[Gemini] Cooling down after rate-limit, skipping request ({remaining}s left)")
        return None
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                cooldown_seconds = int(os.getenv("GEMINI_COOLDOWN_SECONDS", "120"))
                _gemini_cooldown_until = time.time() + max(10, cooldown_seconds)
                print(f"[Gemini] Rate limited (429). Entering cooldown for {max(10, cooldown_seconds)}s " "and falling back without retry.")
                return None
...
def rank_candidates(concept: str, candidates: list) -> list:
    ...
    for i, c in enumerate(top_candidates):
        candidate_list.append(f"{i+1}. {c.name} - {c.description[:100]}... (source: {c.source}, tags: {', '.join(c.tags[:5])})")
