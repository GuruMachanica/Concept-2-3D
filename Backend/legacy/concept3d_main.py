...
from Backend.concept3d.fallback import build_fallback_payload, generate_fallback
from Backend.concept3d.generative_stack import get_ml_status
from Backend.concept3d.hybrid_pipeline import run_hybrid_pipeline
from Backend.concept3d.model_labeling import get_cached_labels
from Backend.concept3d.rag_feedback import (
    get_rag_search_enhancement,
    get_rag_source_recommendations,
    submit_rag_feedback,
)
from Backend.concept3d.recursive_trainer import (
    get_training_status,
    run_manual_training_cycle,
    start_recursive_training,
    stop_recursive_training,
)
from Backend.concept3d.vision import classify_image
from Backend.concept3d.wikipedia_api import get_wikipedia_summary

# Load local .env for concept3d when present
...
FREE_AI_API_MODEL = os.getenv("FREE_AI_API_MODEL", "openai/gpt-oss-20b:free").strip()
FREE_AI_API_URL = os.getenv("FREE_AI_API_URL", "https://openrouter.ai/api/v1/chat/completions").strip()
BACKEND_BASE_URL = os.getenv("BACKEND_BASE_URL", "http://localhost:8000").strip().rstrip("/")
HYBRID_PIPELINE_TIMEOUT_SECONDS = int(os.getenv("HYBRID_PIPELINE_TIMEOUT_SECONDS", "20"))
...
def set_labels(model_id: str, payload: PartLabelsRequest):
    ...
    if payload.auto_generate and payload.concept:
        ...
        return {
            "ok": False,
            "error": "Either part_labels or auto_generate+concept required",
        }
...
def _final_similarity_score(concept: str, model_name: str, description: str, tags: list[str] | None = None) -> float:
    """Compute final concept/result similarity on a 0-1 scale for UI transparency."""
    concept_norm = _normalize_text(concept)
    blob = " ".join([model_name or "", description or "", " ".join(tags or [])])
    blob_norm = _normalize_text(blob)
    if not concept_norm or not blob_norm:
        return 0.0

    semantic = SequenceMatcher(None, concept_norm, blob_norm).ratio()
    c_tokens = set(_keywords(concept_norm))
    b_tokens = set(_keywords(blob_norm))
    overlap = (len(c_tokens.intersection(b_tokens)) / max(1, len(c_tokens))) if c_tokens else 0.0
    phrase_bonus = 1.0 if concept_norm in blob_norm else 0.0

    score = (semantic * 0.45) + (overlap * 0.45) + (phrase_bonus * 0.10)
    return round(max(0.0, min(1.0, score)), 4)


def _model_id_from_url(model_url: str | None) -> str:
    """Extract model file id (filename) from a model URL."""
    if not model_url:
        return ""
    try:
...
def _get_or_generate_part_labels(model_id: str, concept: str, model_name: str) -> dict:
    """Return cached labels when available; otherwise generate and persist labels."""
    if not model_id:
        return {}
    ...
    model_path = os.path.join(models_dir, model_id)
    generated = ai_label_parts(model_path, concept, model_name)
    if generated and generated.get("parts"):
        save_part_labels(model_id, generated)
        return generated
...
def _detect_question_intent(question: str) -> str:
    ...
    definition_starts = ("what is", "what are", "define", "meaning of", "what does")
...
def _best_context_sentence(question: str, context: str) -> str:
    ...
    for sentence in sentences:
        ...
        exact_matches = sum(1 for token in question_keywords if re.search(rf"\b{re.escape(token)}\b", sentence_lower))
        ...
        score = (exact_matches * 2.5) + (partial_matches * 0.6) + (coverage * 2.0) + (similarity * 1.5) + intent_bonus
...
def _ask_free_ai(
    ...
    system_prompt = "You are a concise helpful assistant. " "Answer naturally and directly. " "If uncertain, say you are not sure instead of making up facts. " "Do not mention instructions, sources, or internal reasoning."

    model_line = f"Generated model: {model_name}\n" if model_name else ""
    user_prompt = f"Concept: {concept}\n" f"{model_line}" f"Question: {question}\n\n" "Return only the final answer text."
    ...
    try:
        ...
        if response.status_code == 404 and "guardrail restrictions" in lower_error:
            print("OpenRouter request blocked by account privacy/guardrail settings. " "Update https://openrouter.ai/settings/privacy and retry.")
            return None

        print(f"Free AI API unavailable ({response.status_code}). " "Falling back to Wikipedia-only mode.")
        return None
...
def visualize(concept: str):
    ...
    try:
        ...
        hybrid_result = future.result(timeout=HYBRID_PIPELINE_TIMEOUT_SECONDS)
    except FutureTimeout:
        print(f"[Visualize] Hybrid pipeline timed out after {HYBRID_PIPELINE_TIMEOUT_SECONDS}s " f"for concept='{concept}'. Falling back.")
        hybrid_result = {
            "type": "generated",
            "model_url": None,
            "metadata": {
                ...
                "name": concept.title(),
                "description": "Fallback generated due to timeout",
                "tags": [],
                "format": "none",
            },
        ...
        print(f"[Visualize] Pipeline execution failed: {e}. Falling back.")
        hybrid_result = {
            "type": "generated",
            "model_url": None,
            "metadata": {
                ...
                "confidence_score": 0.0,
                "name": concept.title(),
                "description": "Fallback generated due to pipeline error",
                "tags": [],
                "format": "none",
            },
            "shapes": generate_fallback(concept),
        ...
        # Do not block request completion waiting for long-running upstream tasks.
    ...
    if hybrid_result.get("model_url"):
        ...
        model_id = _model_id_from_url(hybrid_result.get("model_url"))
        final_similarity = _final_similarity_score(concept, model_name, description, tags)

        labels = _get_or_generate_part_labels(
            model_id=model_id,
            concept=concept,
            model_name=model_name,
        )
        ...
        metadata["part_labels_count"] = len(labels.get("parts", [])) if isinstance(labels, dict) else 0
        metadata["final_similarity_score"] = final_similarity
    ...
    fallback_similarity = _final_similarity_score(
        concept,
        concept.title(),
        "Primitive fallback representation",
        [],
    )

    # Generate labels for fallback concept even without a 3D model
    fallback_labels = _get_or_generate_part_labels(
        model_id=f"fallback_{concept.lower().replace(' ', '_')}",
        concept=concept,
        model_name=concept.title(),
    )
    ...
    return {
        ...
        "metadata": {
            ...
            "tags": [],
            "format": "none",
            "final_similarity_score": fallback_similarity,
        },
        "similarity_score": fallback_similarity,
        "shapes": hybrid_result.get("shapes", fallback_payload.get("shapes", generate_fallback(concept))),
        "geometry_details": fallback_payload.get("geometry_details", {}),
        "fallback_2d_image_url": fallback_payload.get("fallback_2d_image_url"),
        "fallback_2d_source": fallback_payload.get("fallback_2d_source", "procedural_blueprint"),
        "part_labels": fallback_labels,
        "ai_overview": ai_overview,
    }
...
def ml_diagnostics():
    """
    Comprehensive diagnostics for ML generation pipeline (SD + OpenLRM).
    Use this to debug why generation is failing.
    """
    import os
    ...
    diagnostics = {**status, "issues": [], "recommendations": [], "checks": {}}

    # Check 1: Device availability
    try:
        import torch

        diagnostics["checks"]["torch_available"] = True
        diagnostics["checks"]["cuda_available"] = torch.cuda.is_available()
        if torch.cuda.is_available():
            diagnostics["checks"]["gpu_name"] = torch.cuda.get_device_name(0)
    except ImportError:
        diagnostics["checks"]["torch_available"] = False
        diagnostics["issues"].append("PyTorch not installed - required for ML generation")
        diagnostics["recommendations"].append("Install PyTorch: pip install torch torchvision")

    # Check 2: OpenLRM setup
    if not status["openlrm_repo_exists"]:
        diagnostics["issues"].append(f"OpenLRM repo not found at {status.get('openlrm_repo_dir', 'unknown')}")
        diagnostics["recommendations"].append(f"Clone OpenLRM: cd {os.path.dirname(__file__)}/ml && git clone https://github.com/your-org/OpenLRM.git")

    if not status["openlrm_launch_exists"]:
        diagnostics["issues"].append("OpenLRM launch.py not found - repo may be incomplete")
        diagnostics["recommendations"].append("Verify OpenLRM repo has openlrm/launch.py")

    # Check 3: Model downloads
    diagnostics["checks"]["hf_models_cached"] = status.get("hf_cache_files", 0) > 0
    if not diagnostics["checks"]["hf_models_cached"]:
        diagnostics["recommendations"].append("First ML generation will download models (~10-20GB) - be patient")

    # Check 4: Required packages
    packages_to_check = {
        "diffusers": "Stable Diffusion images",
        "rembg": "Background removal (optional but recommended)",
        "trimesh": "Mesh format conversion",
        "PIL": "Image processing",
    }

    for pkg_name, purpose in packages_to_check.items():
        try:
            __import__(pkg_name)
            diagnostics["checks"][f"{pkg_name}_available"] = True
        except ImportError:
            diagnostics["checks"][f"{pkg_name}_available"] = False
            diagnostics["issues"].append(f"{pkg_name} not installed (needed for {purpose})")

    # Check 5: Configuration
    if not os.getenv("GENERATOR_ENABLED", "true").lower() == "true":
        diagnostics["issues"].append("ML generation disabled - GENERATOR_ENABLED=false in .env")

    if status["device"] == "unknown":
        diagnostics["issues"].append("Device resolution failed - check torch/cuda installation")
