    ...
    if any(
        ...
        return [
            ...
            },
            {
                ...
                "position": {"x": 0.25, "y": 0.02, "z": 0.0},
                "description": "Innermost rocky planet.",
            },
            {
                "name": "venus",
                "primitive": "sphere",
                ...
                "position": {"x": 0.45, "y": 0.02, "z": 0.03},
                "description": "Second planet with dense atmosphere.",
            },
            {
                "name": "earth",
                "primitive": "sphere",
                ...
                "position": {"x": 0.62, "y": 0.0, "z": 0.0},
                "description": "Third planet and home world.",
            },
            {
                "name": "mars",
                "primitive": "sphere",
                ...
                "position": {"x": 0.78, "y": -0.01, "z": 0.04},
                "description": "Fourth rocky planet.",
            },
            {
                "name": "jupiter",
                "primitive": "sphere",
            ...
            },
            {
                ...
                "position": {"x": 1.28, "y": 0.02, "z": 0.03},
                "description": "Gas giant known for rings.",
            },
            {
                "name": "outer_orbit_band",
                "primitive": "tube",
                "parameters": {
                    "radiusTop": 0.08,
                    "radiusBottom": 0.08,
                ...
                "position": {"x": 0.82, "y": -0.25, "z": 0.0},
                "description": "Simplified orbital path band for outer planets.",
            },
        ]
...
def _draw_shape(draw, shape: str, x: int, y: int, size: int, color):
    if shape == "cube":
        ...
        draw.line(
            (x - size, y - size * 3 // 4, x - size, y + size * 3 // 4),
            fill=color,
            width=3,
        )
        draw.line(
...
def _generate_svg_fallback_image(concept: str, models_dir: str, backend_base_url: str):
    ...
    key = hashlib.md5(json.dumps({"concept": concept, "type": "svg_fallback", "version": 1}, sort_keys=True).encode("utf-8")).hexdigest()[:12]
...
def _get_wikipedia_summary_image_url(concept: str, query_context: str | None = None):
    ...
    headers = {"User-Agent": "Concept3D-Generative/1.0 (Educational AI model generation; contact: support@example.com)"}
...
def _generate_concept_image_from_pollinations(concept: str, models_dir: str, backend_base_url: str):
    ...
    key = hashlib.md5(json.dumps({"concept": concept, "type": "pollinations", "version": 1}, sort_keys=True).encode("utf-8")).hexdigest()[:12]
