...
class RAGFeedbackStore:
    ...
    def retrieve_similar_feedback(
        ...
        try:
            ...
            good_feedback = list(collection.find({"rating": {"$gte": min_rating}}).limit(100))

            # Calculate similarity scores
            scored_feedback = []
            for feedback in good_feedback:
                # Combine embedding similarity and text similarity
                emb_sim = self._cosine_similarity(query_embedding, feedback.get("concept_embedding", []))
                ...
                if combined_sim >= similarity_threshold:
                    scored_feedback.append({**feedback, "similarity_score": combined_sim})
            ...
            for item in results:
                collection.update_one({"_id": item["_id"]}, {"$inc": {"retrieval_count": 1}})
    ...
    def get_search_enhancement(self, concept: str) -> Dict[str, Any]:
        ...
        if similar_feedback:
            ...
            enhancement["recommended_sources"] = sorted(source_recs.keys(), key=lambda x: source_recs[x], reverse=True)
...
def submit_rag_feedback(
    ...
    store = get_rag_store()
    return store.store_feedback_with_embedding(concept, model_id, model_source, rating, user_feedback, search_params)
