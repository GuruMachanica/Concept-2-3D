...
class ModelSearchEngine:
    def __init__(self):
        ...
        self.high_probability_index = self._load_high_probability_index()
    ...
    def _get_gemini_label_positions(
        ...
        try:
            ...
            parts_list = "\n".join([f"- {i+1}. {p.get('name', 'Part')} (Description: {p.get('description', 'N/A')})" for i, p in enumerate(part_definitions[:10])])  # Limit to 10 parts
            ...
            return [
                {
                    "name": "head",
                    "primitive": "sphere",
                    "description": "Bear head containing eyes, nose and jaw.",
                    "position": {"x": 0.0, "y": 0.32, "z": 0.22},
                    "parameters": {},
                },
                {
                    "name": "torso",
                    "primitive": "cube",
                    "description": "Main body mass housing chest and abdomen.",
                ...
                },
                {
                    "name": "front_legs",
                    "primitive": "cylinder",
                    "description": "Forelimbs used for support and movement.",
                    "position": {"x": -0.2, "y": -0.25, "z": 0.1},
                    "parameters": {},
                },
                {
                    "name": "hind_legs",
                    "primitive": "cylinder",
                    "description": "Rear limbs providing propulsion.",
                    "position": {"x": 0.22, "y": -0.25, "z": -0.08},
                    "parameters": {},
                },
                {
                    "name": "snout",
                    "primitive": "cone",
                    "description": "Forward facial structure containing nose and mouth.",
                    "position": {"x": 0.0, "y": 0.24, "z": 0.34},
                    "parameters": {},
                },
            ]

        category_prior_parts = get_category_part_priors(query)
        if category_prior_parts:
            return category_prior_parts

        return []

    def _is_realistic_intent(self, keywords: str):
        query = (keywords or "").lower()
        stylized_hints = {
            "cartoon",
            "anime",
            "stylized",
        ...
        return not any(hint in query for hint in stylized_hints)

    def _realism_adjustment(self, keywords: str, title: str = "", tags: list | None = None):
        realistic = self._is_realistic_intent(keywords)
        blob = f"{title or ''} {' '.join([str(t) for t in (tags or [])])}".lower()
        ...
        stylized_hits = sum(1 for t in stylized_terms if t in blob)
        realistic_hits = sum(1 for t in realistic_terms if t in blob)

        if realistic:
            return realistic_hits * 3.0 - stylized_hits * 6.0
        return stylized_hits * 3.0

    def _generate_ai_fallback_parts(self, normalized_keywords: str, max_parts: int = 8):
        # Query-specific templates are still highest precision.
        ...
        try:
            prompt = f"""
Generate realistic structural parts for this object: {normalized_keywords}
...
{{
  "parts": [
    {{
      "name": "part_name",
      "primitive": "cube|sphere|cylinder|cone|tube",
      "description": "Short precise definition",
      "position": {{"x": 0.0, "y": 0.0, "z": 0.0}},
      "parameters": {{}}
    }}
  ]
...
Rules:
- 4 to 8 parts max
- Only object-relevant parts, no generic terms like model/object/shape/view
...
"""
            ...
            cleaned = []
            for idx, p in enumerate(parts[:max_parts]):
                if not isinstance(p, dict):
                    continue
                name = (p.get("name") or f"part_{idx + 1}").strip().lower().replace(" ", "_")
                if not name:
                    continue
                primitive = (p.get("primitive") or "sphere").strip().lower()
                if primitive not in {"cube", "sphere", "cylinder", "cone", "tube"}:
                    primitive = "sphere"
                desc = (p.get("description") or f"Structural part: {name}").strip()
                pos = p.get("position") if isinstance(p.get("position"), dict) else {}
                x = max(-0.5, min(0.5, float(pos.get("x", 0.0))))
                y = max(-0.5, min(0.5, float(pos.get("y", 0.0))))
                z = max(-0.5, min(0.5, float(pos.get("z", 0.0))))
            ...
            print(f"AI fallback part generation failed for '{normalized_keywords}': {e}")
            return []

    def _dynamic_part_definitions(
        self,
        normalized_keywords: str,
    ...
    def _labels_need_fallback(self, normalized_keywords: str, part_definitions: list):
        ...
        combined = " ".join([f"{p.get('name', '')} {p.get('description', '')}" for p in part_definitions if isinstance(p, dict)]).lower()
        ...
        if "heart" in query_tokens:
            ...
            names = {(p.get("name") or "").strip().lower() for p in part_definitions if isinstance(p, dict)}
    ...
    def _fetch_concept2_labeled_model(self, normalized_keywords: str, base_score: float = 81.0):
        ...
        try:
            ...
            title = (metadata.get("name") if isinstance(metadata, dict) else None) or model_data.get("name") or normalized_keywords.title()
            description = (metadata.get("description") if isinstance(metadata, dict) else None) or model_data.get("description") or f"External hybrid model + labels for '{normalized_keywords}'."
    ...
    def _load_high_probability_index(self):
        path = os.path.join(self.models_dir, "high_probability_model_index.json")
        ...
        try:
            ...
            print(f"Failed to load high probability model index: {e}")
            return []

    def _search_high_probability_index(self, normalized_keywords: str, limit: int = 6):
        if not self.high_probability_index:
            return []

        query_tokens = set(self._tokenize(normalized_keywords))
        generic_tokens = {
            "system",
            "model",
            "models",
        ...
        effective_query_tokens = {t for t in query_tokens if t not in generic_tokens}
        if not query_tokens:
            return []

        solar_intent = ("solar" in query_tokens and "system" in query_tokens) or ("planet" in query_tokens)
        solar_domain_terms = {
            "solar",
            "sun",
            "planet",
        ...
        query_categories = set(detect_categories(normalized_keywords))
        scored = []

        for row in self.high_probability_index[:5000]:
            ...
            blob = f"{title} {description} {' '.join([str(t) for t in tags])} {category}".lower()
            blob_tokens = set(self._tokenize(blob))
            active_query_tokens = effective_query_tokens or query_tokens
            overlap = len(active_query_tokens & blob_tokens)
            if overlap == 0:
                continue

            overlap_ratio = overlap / max(1, len(active_query_tokens))
            quality = float(row.get("quality_score") or 68.0)
            realism = float(self._realism_adjustment(normalized_keywords, title, tags))
            category_boost = 6.0 if category and category in query_categories else 0.0
            domain_boost = 0.0
            if solar_intent:
                domain_hits = len(solar_domain_terms & blob_tokens)
                domain_boost += domain_hits * 3.0
                if "solar" not in blob_tokens and "planet" not in blob_tokens and "sun" not in blob_tokens:
                    domain_boost -= 10.0

            total_score = 58.0 + (overlap_ratio * 22.0) + (quality * 0.25) + realism + category_boost + domain_boost
        ...
        for score, row in scored[:limit]:
            uid = row.get("uid") or f"seed-{abs(hash(row.get('title', 'item')))}"
            cards.append(
                {
                    "source": "Category Index (HQ)",
                    "title": row.get("title") or "Indexed model",
                    "uid": f"seed-{uid}",
                    "thumbnails": row.get("thumbnails") or [],
                    "model_url": row.get("model_url"),
                    "embed_url": row.get("embed_url"),
                    "score": max(50.0, min(96.0, float(score))),
                    "explanation": (f"High-probability indexed model selected for '{normalized_keywords}' " f"from large category prefetch set."),
                    "index_quality_score": row.get("quality_score"),
                }
            )
    ...
    def _build_labeled_breakdown_model(self, normalized_keywords: str, base_score: float = 82.0):
        ai_parts = self._generate_ai_fallback_parts(normalized_keywords, max_parts=8)
        fallback_payload = build_fallback_payload(normalized_keywords)
        geometry_details = (fallback_payload or {}).get("geometry_details") or {}
        parts = ai_parts or geometry_details.get("shapes") or []
    ...
    def _build_realistic_fallback_model(self, normalized_keywords: str, base_score: float = 79.0):
        card = self._build_labeled_breakdown_model(normalized_keywords, base_score=base_score)
        card["source"] = "Realistic Fallback 3D"
        card["title"] = f"Realistic Fallback: {normalized_keywords.title()}"
        card["uid"] = f"realistic-fallback-{normalized_keywords.replace(' ', '-')}"
        card["explanation"] = f"Fallback procedural model for '{normalized_keywords}' with explicit labeled anatomy for reliable learning."
        card["labeling_mode"] = "point-based-fallback"
        return card

    def _build_original_labeled_test_card(
        self,
        normalized_keywords: str,
        base_model: dict,
        base_score: float = 81.0,
        intent_data: dict | None = None,
    ):
    ...
    def search(self, intent_data: dict) -> list:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            future_seed_index = executor.submit(self._search_high_probability_index, normalized_keywords)
        ...
        if not results:
            ...
            ai_parts = self._generate_ai_fallback_parts(normalized_keywords, max_parts=8)
            parts = ai_parts or geometry_details.get("shapes") or []
            components = [p.get("primitive") for p in parts if isinstance(p, dict) and isinstance(p.get("primitive"), str)]
        ...
        # Guarantee point-based labels for fallback-only cards.
        for model in results:
            is_fallback_card = bool(model.get("procedural_data")) or "fallback" in (model.get("source", "").lower())
            if is_fallback_card:
                self._ensure_point_based_labels(model, normalized_keywords, intent_data=intent_data)

        # Ensure at least one explicit realistic fallback card is always available.
        if results:
            has_retrieved_model = any((not m.get("procedural_data")) and (m.get("model_url") or m.get("embed_url")) for m in results)
            top_score = float(results[0].get("score") or 82)
            has_realistic_fallback = any((m.get("uid") or "").startswith("realistic-fallback-") for m in results)

            if not has_realistic_fallback:
                if has_retrieved_model:
                    fallback_score = max(68.0, min(84.0, top_score - 8.0))
                else:
                    fallback_score = max(72.0, min(88.0, top_score - 2.0))
                results.append(self._build_realistic_fallback_model(normalized_keywords, base_score=fallback_score))
            ...
            if response.status_code == 200:
                ...
                for uid, data in assets.items():
                    ...
                    if match:
                        ...
                        try:
                            ...
                            if file_res.status_code == 200:
                                ...
                                if gltf_data:
                                    ...
                                    res_key = min(
                                        gltf_data.keys(),
                                        key=lambda k: (int(k.replace("k", "")) if k.replace("k", "").isdigit() else 99),
                                    )
                        ...
                        realism_bonus = self._realism_adjustment(
                            " ".join(keywords),
                            data.get("name", ""),
                            data.get("tags", []),
                        )
                        ...
                        results.append(
                            {
                                ...
                                "model_url": model_url,
                                "embed_url": None,
                                "score": max(50, min(100, 90 + realism_bonus)),
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for item in data.get("results", [])[:5]:  # Get slightly more for sorting
                    ...
                    dynamic_score += self._realism_adjustment(keywords, item.get("name", ""), tags)
    ...
    def _generate_tripo3d(self, keywords: str) -> list:
        ...
        payload = {"type": "text_to_model", "prompt": keywords}
