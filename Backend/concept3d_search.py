...
def _score_candidate(query, tokens, result):
    ...
    relevance_score = (exact_matches * 2.8) + (partial_matches * 0.6) + (token_coverage * 2.5) + (name_similarity * 2.0) + phrase_bonus
    ...
    is_relevant = phrase_match_in_name or (exact_matches >= strict_min_exact and name_or_tag_matches >= 1) or (len(tokens) <= 1 and token_coverage >= 0.6)
    ...
    try:
        ...
        for score_data, top_match, target_gltf_file in ranked_candidates:
            ...
            try:
                ...
                download_endpoint = f"https://www.blenderkit.com/api/v1/downloads/{download_id}/" f"?scene_uuid={dummy_scene_uuid}"

                print(f"Selected BlenderKit model '{top_match.get('name', 'unknown')}' " f"with relevance={score_data['relevance_score']:.2f} and exact_matches={score_data['exact_matches']}")
                ...
                print(f"Download failed for candidate '{top_match.get('name', 'unknown')}'. " f"Trying next candidate. Error: {error}")
