...
class RecursiveTrainer:
    ...
    def run_training_cycle(self) -> TrainingMetrics:
        ...
        training_data = get_training_batch(limit=100)
        ...
        entry_ids = [item.get("_id") for item in training_data if item.get("_id") is not None]
        if entry_ids:
            mark_training_processed(entry_ids)
        ...
        print("[RecursiveTrainer] ===== Cycle Complete =====")
    ...
    def _update_search_parameters(self, source_performance: Dict[str, float], threshold_recommendation: float):
        ...
        try:
            ...
            print("[RecursiveTrainer] Updated search parameters")
