...
def _generate_image(prompt: str, key: str) -> str | None:
    ...
    if openlrm_optimized:
        ...
        prompt_final = f"{prompt}, highly detailed 3D model, photorealistic render, PBR materials, crisp geometry, " "studio lighting, 3/4 view and front view, neutral background, no text, no watermark"
...
def _run_openlrm(clean_image_path: str, key: str) -> str | None:
    ...
    if not os.path.exists(launch_py):
        print(f"[OpenLRM] ERROR: launch.py not found at {launch_py}")
        print(f"[OpenLRM] Expected repo dir: {repo_dir}")
        print(f"[OpenLRM] Fix: Clone OpenLRM: git clone https://github.com/your-org/OpenLRM.git {repo_dir}")
        return None
    ...
    print(f"[OpenLRM] Running command (timeout={timeout_s}s): {' '.join(command[:5])}...")
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print(f"[OpenLRM] FAILED with return code {result.returncode}")
            print(f"[OpenLRM] Error output: {stderr_tail}")
            return None
        print(f"[OpenLRM] ✓ Success: mesh generated in {output_dir}")
    except subprocess.TimeoutExpired:
        print(f"[OpenLRM] TIMEOUT after {timeout_s}s - increase GENERATOR_MAX_SECONDS in .env")
        return None
    except Exception as error:
        print(f"[OpenLRM] EXCEPTION: {error}")
        return None
    ...
    if not candidates:
        print(f"[OpenLRM] ERROR: No mesh files (.glb/.obj/.ply) found in output dir {output_dir}")
        return None
...
def generate_ml_glb(concept: str, models_dir: str) -> dict[str, Any] | None:
    if not _env_bool("GENERATOR_ENABLED", True):
        print("[ML] Generator disabled via GENERATOR_ENABLED=false")
        return None
    ...
    if os.path.exists(model_path):
        print(f"[ML] Using cached ML-generated model: {model_filename}")
        return {
    ...
    print(f"[ML] Generating 3D model for '{concept}' (device={_resolve_device()})")

    image_path = _generate_image(concept, key)
    if not image_path:
        print("[ML] FAILED at image generation step - Stable Diffusion returned no image")
        return None

    print(f"[ML] ✓ Generated image: {image_path}")

    clean_image_path = _remove_background(image_path, key)
    print(f"[ML] ✓ Removed background: {clean_image_path}")

    mesh_path = _run_openlrm(clean_image_path, key)
    if not mesh_path:
        print("[ML] FAILED at OpenLRM step - check OpenLRM repo installation and model download")
        return None

    print(f"[ML] ✓ OpenLRM generated mesh: {mesh_path}")

    success = _convert_mesh_to_glb(mesh_path, model_path)
    if not success:
        print(f"[ML] FAILED at mesh conversion - could not convert {mesh_path} to GLB")
        return None

    print(f"[ML] ✓ Successfully saved GLB: {model_filename}")
    return {
        "filename": model_filename,
        "source": "ml_openlrm",
        "details": {
...
def generate_ml_preview_image(concept: str, models_dir: str) -> dict[str, Any] | None:
    """Generate a 2D concept image and copy it into models dir for static serving."""
    if not concept:
        return None
    if not _env_bool("GENERATOR_ENABLED", True):
        return None
    ...
    try:
        ...
        print(f"[ML] Failed to copy preview image into models dir: {error}")
        return None
