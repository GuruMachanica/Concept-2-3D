...
ENABLE_ML_FALLBACK = os.getenv("ENABLE_ML_FALLBACK", "false").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def _sketchfab_candidates_with_state(concept: str, api_state: dict | None) -> list[Candidate]:
    ...
    search_url = "https://api.sketchfab.com/v3/search" f"?type=models&q={query_encoded}&downloadable=true&count=24"
...
def _composite_score(concept: str, candidate: Candidate, use_gemini: bool = True) -> float:
    ...
    score = semantic * 0.45 + overlap * 0.30 + phrase * 0.15 + quality * 0.10  # Increased weight for semantic accuracy  # Token overlap with stricter penalties  # Phrase matching (now stricter)  # Source quality signal
...
def _download_and_cache_glb(candidate: Candidate, models_dir: str) -> str | None:
    ...
    try:
        if candidate.source == "blenderkit":
            ...
            has_downloadable_file = any(f.get("fileType") in ("glb", "gltf", "gltf_godot") for f in files)
...
def run_hybrid_pipeline(concept: str, models_dir: str, backend_base_url: str) -> dict[str, Any]:
    ...
    print("[Gemini] Enhancing top 5 candidates with AI scoring...")
    enhanced_ranked = []
    ...
    for confidence, candidate in ranked:
        ...
        if not _candidate_matches_required_tokens(concept, candidate):
            print(f"Skipping candidate due to missing required concept tokens: " f"{_required_concept_tokens(concept)}")
            continue
        # Skip non-GLB formats from BlenderKit (we can't display .blend files)
        if candidate.source == "blenderkit" and candidate.format_type not in (
            "glb",
            "gltf",
        ):
