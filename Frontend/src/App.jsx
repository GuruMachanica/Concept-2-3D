...
function App() {
  ...
  const handleImageUpload = async (file) => {
    ...
    setError(null);
    try {
      // Use async job endpoint so long-running TripoSR runs don't block UI
      ...
      let attempts = 0;
      let jobResp = null;
      while (attempts < 120) { // up to ~4 minutes with 2s interval
        ...
        try {
          ...
          // continue polling unless server explicitly reports failure
        }
      }
    ...
    <div className="min-h-screen flex flex-col items-center bg-[#0d1424] overflow-hidden text-slate-100 p-4">
      ...
      <main className="w-full max-w-[1440px] flex-grow flex flex-col z-10 relative overflow-hidden">
        ...
        <div className="w-full max-w-2xl mx-auto mb-6">
          <SearchBar onSearch={handleSearch} onImageUpload={handleImageUpload} isLoading={isLoading} />
        </div>
